...
const Stem = (word) => {
  stemmedWord = GetPhoneticWord(word);
  for (fix of Prefix) {
    if (stemmedWord.startsWith(fix) && stemmedWord.length > 2)
      stemmedWord = stemmedWord.replace(fix, "");
  }

  for (fix of Suffix) {
    if (stemmedWord.endsWith(fix) && stemmedWord.length > 2)
      stemmedWord = stemmedWord.replace(fix, "");
  }
