...
def get_pages():
    for i in tqdm(range(0, 20), desc='Fetching blog details'):
        ...
        if i == 0:
            url = 'https://www.ethiopianreporter.com/news/'
        elif i == 1:
            continue
        else:
            url = f'https://www.ethiopianreporter.com/news/page/{i}'
    ...
    with open('ethiopianReporter_stories.json', 'w') as file:
...
async def get_stories():
    urls = []
    blog = []
    with open('ethiopianReporter_stories.json', 'r') as file_:
...
async def write_to_excel():
    ...
    df.to_excel('ethiopianReporterPressRelease.xlsx', sheet_name='news', index=False)
