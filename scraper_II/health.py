...
def get_pages():
    for i in tqdm(range(0, 10), desc='Fetching blog details'):
        if i == 0:
            url = 'https://amharic-zehabesha.com/%E1%88%AD%E1%8B%95%E1%88%B5/%e1%8c%a4%e1%8a%93'
        else:
            url = f'https://amharic-zehabesha.com/%E1%88%AD%E1%8B%95%E1%88%B5/%e1%8c%a4%e1%8a%93/page/{i}'
    ...
    with open('health_stories.json', 'w') as file:
...
async def get_stories():
    urls = []
    blog = []
    with open('health_stories.json', 'r') as file_:
...
async def write_to_excel():
    ...
    # print(df.head())
    df.to_excel('healthPressRelease.xlsx', sheet_name='news', index=False)
